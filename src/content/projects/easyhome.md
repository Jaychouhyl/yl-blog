...
*架构图：依据用户端、管理端、Spring Boot 业务接口、RAG、SSE、Kafka、PostgreSQL/pgvector、Redis 与对象存储整理。图中“话题判断与问题改写”“向量 Top-K”对应当前代码路径；结构化条件合并、完整收藏与预约闭环仍属于改进方向。*
...
项目早期的页面更接近功能原型，我参与的后续优化重点是让不同管理页面拥有一致的布局、表格、表单和操作入口。当前仓库中的房源管理、用户管理和预约管理页面仍使用本地模拟数据，页面操作主要修改前端数组；后端虽有部分审核与预约接口，但不能据此把这些页面写成已经完成真实联调。这段工作后来也影响了我在 SmartX ERP 中对企业后台设计系统的理解。

## 2. 为什么自然语言适合补充传统房源筛选

传统筛选器适合处理价格、面积、户型和区域等结构化条件，却很难直接表达“适合居家办公”“老人出行方便”“晚上比较安静”等软性需求。LLM 的价值在于理解这些自然语言意图，但模型不能凭空知道当前有哪些真实房源。

RAG 在这里承担了一条连接链路：先判断话题并改写用户问题，再从房源向量数据中检索候选，最后把房源信息作为上下文交给模型生成回答。这样可以减少模型完全依赖参数记忆的问题，也让推荐理由有机会回到具体房源信息。
...
要让流式回答真正可用，前端仍需处理增量拼接、重复片段、异常中断、重新提问和页面状态恢复。当前用户端已经能从回答中解析房源 ID，并按 ID 加载详情卡片；但收藏回调仍未形成完整业务写入，预约与状态反馈也没有在本次复盘中完成端到端验证。因此可以写“回答已连接房源卡片”，不能把后续收藏和预约一并写成完成闭环。
