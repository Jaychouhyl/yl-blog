...
项目以本地开发和演示为主，系列文章中简称 SmartX ERP。截图、单据、客户、供应商和财务数据均为虚拟数据，不将系统描述为已经生产上线、通过大规模压测或产生了可量化业务收益的商业产品。开发周期为 2025-12-25 至 2026-08-20；下文引用的测试与运行记录主要来自 2026-07-26 归档，不代表当前环境已整体复验。
...
图中展示的是定位成功时的目标流程，不表示每次点击都能找到唯一记录。订单类实体通过业务 ID 或编号尝试打开详情；客户和供应商按 ID 或名称加载候选，定位结果分为 found、missing 和 ambiguous，不能静默选择第一条同名记录。

当前实现同时使用实体字段、文本规则和原文区间校验，并非完全不做文本推断。主数据定位中的 ID 查询异常会转入名称回退，没有在该函数内分别保留网络错误、无权限与记录不存在的原因；唯一性也只针对返回的候选集，不代表全库唯一。前端定位仍不能代替后端权限校验，具体示例见[实体路由复盘](../../posts/smartx-erp-rag-entity-routing/)。
...
Key 只在创建时显示一次，数据库保存 SHA-256 哈希和可识别前缀。MCP 不使用固定的超级管理员身份访问所有数据，而是先把 Key 解析为真实用户，再向 Auth 服务申请内部令牌。下游服务继续执行原有的角色权限和部门数据范围。已有员工上下文但令牌签发失败时，应拒绝调用，而不是静默退回 bot 身份；内部签发请求则需避开用户令牌拦截器的自举递归。

### 6.3 双层审计

工具调用记录回答了“调用了哪个工具、由谁调用、访问了什么业务能力”；模型调用记录用于核对模型、会话、工具参数、返回结果与最终答案。两类记录分别写入 `mcp:invocation:log` 和 `rag:model:call:audit`。这里记录的是接口可见的调用过程，不是模型内部思考；流程 success 也不等于答案已经过事实验证。

模型审计在异常路径中也会尝试写入，但写入失败只记录警告，不能保证每次问答都成功留档。Redis Stream 裁剪与字段长度限制还会影响可追溯范围；可选文件归档不等于长期留存、访问控制和恢复流程已完成验收。排错方法及保存范围见[模型审计复盘](../../posts/smartx-erp-agent-audit/)。
...
RocketMQ 用于允许延迟处理的提醒和派生事件。不过，当前采购财务提醒并非纯异步流程：先远程调用 Finance 创建提醒，再在 SCM 保存 Outbox 记录，并在同一次服务调用中尝试发消息。SCM 侧记录的消息状态为：

`PENDING → SENT`，失败时进入 `FAILED` 并在重试边界内继续处理。

Finance 侧记录消费失败和死信状态，并提供排查入口。Outbox 的价值是留下可观察、可重试的事件，但 Finance 远程写入不属于 SCM 本地事务，发送也没有完全交给独立 Relay；因此这还不是标准 Transactional Outbox 的完整原子链路。远程已成功、本地后失败时仍需核对两侧状态，不能依靠本地回滚撤销远程写入。

### 7.3 幂等与对账

幂等需要按业务动作分别判断，不能用一个总开关概括。采购提醒使用 `PURCHASE_PAYMENT:<采购单 ID>` 作为事件键；Finance 创建提醒时复用同一业务的 PENDING / PROCESSING 记录，已完成提醒不在该条件内。“先查后写”也不能单独证明并发请求不会重复插入。

当前对账查询检查一批 DONE 提醒是否存在对应的 CONFIRMED 流水，返回差异线索，不自动修复，也不覆盖全部库存、订单和财务差异。同步事务、重试、消费幂等和对账各有范围，异常恢复能力仍需通过故障注入确认；调用顺序与失败推演见[一致性策略复盘](../../posts/smartx-erp-consistency-strategy/)。
...
当前 HMAC 签名覆盖调用方、时间戳、方法和路径，不包含请求正文与 nonce。时间窗口可以拒绝过期请求，但不是完整的重放防护；不能把签名存在等同于请求所有内容都受到保护。nonce、mTLS、密钥轮换和长期审计治理仍是待完善方向，具体边界见[MCP 身份与权限复盘](../../posts/smartx-erp-mcp-boundary/)。
...
因此 UI 优化逐步沉淀为共享 Hero、筛选表面和主题变量，而不是继续对每个页面做孤立的像素修补。业务成功、警告、危险和完成状态仍保留语义颜色，视觉统一不等于抹平业务含义。

展示层的容错也有边界：把乱码回退为占位文本，没有修复原始数据；筛选栏发出 reset 事件，不等于已经清空业务条件或重新查询。Aeria 的容器规则与组件源码复用帮助统一体验，但不能据此认定所有页面都已接入，或上游更新能够自动合并。具体实现分别在前端体验与 Aeria 两篇复盘中展开。
...
- 同日期归档的 Surefire 报告汇总为后端 374 项通过；这是独立的测试证据，不是下述跳过测试的构建执行结果。
